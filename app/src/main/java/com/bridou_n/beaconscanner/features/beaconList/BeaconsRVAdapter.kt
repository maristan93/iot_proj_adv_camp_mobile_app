...
typealias CreateCoupon = (uuid: String) -> Unit

class BeaconsRecyclerViewAdapter(
        val clickListener: OnControlsOpen?,
        val createCoupon: CreateCoupon?
    ...
    class BeaconViewHolder(itemView: View, val listener: OnControlsOpen?, val couponListener: CreateCoupon?) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bindView(beacon: BeaconSaved) {
            ...
            beacon.ibeaconData?.uuid?.let { uuid ->
                itemView.setOnClickListener { couponListener?.invoke(uuid) }
                itemView.infos_rv.setOnClickListener { couponListener?.invoke(uuid) }
            }
            ...
            val beaconInfos = mutableListOf<BeaconInfo>().apply {
                ...
                beacon.couponData?.let {
                    add(BeaconInfo("Name", it.name))
                    add(BeaconInfo("Type", it.section))
                }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            R.layout.item_beacon -> BeaconViewHolder(view, clickListener, createCoupon)
